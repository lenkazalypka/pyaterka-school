...
-- subscription_subjects already has PK (subscription_id, subject_id), so an
-- additional subscription_id index would duplicate the useful leading key.
create index if not exists groups_program_idx on public.groups(program_id);
create index if not exists modules_program_idx on public.modules(program_id);
create index if not exists topics_module_idx on public.topics(module_id);
create index if not exists curator_students_student_idx on public.curator_students(student_id);
