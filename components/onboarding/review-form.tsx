...
export function ReviewForm({ data, idempotencyKey }: { data:ReviewData; idempotencyKey:string }) {
  const planPrice=data.plan.pricesMinor[data.subjects.length]??null;
  return <FormShell action={completeOnboarding} submitLabel="Завершить онбординг" hidden={<input type="hidden" name="idempotencyKey" value={idempotencyKey}/>}>
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Section title="Тариф" href="/onboarding/plan"><p className="text-xl font-extrabold text-[var(--text-primary)]">{data.plan.name}</p><p>{planPrice?`${money(planPrice,data.plan.currency)} / месяц · ${data.subjects.length} предмета`:"Цена для выбранной комбинации не опубликована"}</p><p className="mt-2">Подписка будет создана со статусом «ожидает ручной активации». Оплата не считается произведённой.</p></Section>
    </div>
