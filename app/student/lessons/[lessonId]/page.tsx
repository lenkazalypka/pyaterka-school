...
import { CompleteLessonForm, HomeworkSubmissionForm, StartHomeworkForm, StartLessonForm } from "@/components/student-learning-actions";
...
export default async function StudentLessonPage({ params }: { params: Promise<{ lessonId: string }> }) {
  ...
  return <StudentShell identity={identity} active="lessons">
    <div className="student-page student-lesson-page">
      ...
      <header className="student-lesson-hero"><span>{lesson.subject}</span><h1>{lesson.title}</h1>{lesson.description && <p>{lesson.description}</p>}<div>{lesson.event && <span><CalendarDays aria-hidden="true" />{eventDate(lesson.event.startsAt, identity.timezone)}, {eventTime(lesson.event.startsAt, identity.timezone)}</span>}{lesson.teacher && <span><UserRound aria-hidden="true" />{lesson.teacher}</span>}</div>{lesson.event?.joinUrl && <a className="button button-primary" href={lesson.event.joinUrl} target="_blank" rel="noreferrer">Подключиться <ExternalLink aria-hidden="true" /></a>}{lesson.progressStatus === null ? <StartLessonForm lessonId={lesson.id} /> : <CompleteLessonForm lessonId={lesson.id} completed={lesson.progressStatus === "completed"} />}</header>

      <div className="student-lesson-layout">
        <div>
          <section className="student-detail-card"><div className="student-card-heading"><div><small>После занятия</small><h2>Запись</h2></div><PlayCircle aria-hidden="true" /></div>{lesson.recording ? lesson.progressStatus === null ? <div className="student-empty"><PlayCircle aria-hidden="true" /><b>Начните урок, чтобы открыть запись</b><p>ELIO сохранит начало работы и будет запоминать позицию просмотра.</p></div> : lesson.recording.sourceType === "private_storage" ? <LessonRecordingPlayer lessonId={lesson.id} src={lesson.recording.watchUrl} title={lesson.recording.title} initialPositionSeconds={lesson.progressPositionSeconds ?? 0} /> : <div className="student-recording"><PlayCircle aria-hidden="true" /><div><b>{lesson.recording.title}</b>{duration(lesson.recording.durationSeconds) && <small>{duration(lesson.recording.durationSeconds)}</small>}</div><a className="button button-primary button-small" href={lesson.recording.watchUrl} target="_blank" rel="noreferrer">Открыть запись</a></div> : <div className="student-empty"><PlayCircle aria-hidden="true" /><b>Запись ещё не опубликована</b><p>Если урок уже прошёл, запись появится после загрузки преподавателем или администратором.</p></div>}</section>
          ...
          <section className="student-detail-card"><div className="student-card-heading"><div><small>После урока</small><h2>Домашнее задание</h2></div><ClipboardCheck aria-hidden="true" /></div>{lesson.assignments.length ? <div className="space-y-5">{lesson.assignments.map((assignment) => <article className="rounded-2xl border border-[var(--border-soft)] p-5" key={assignment.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-extrabold">{assignment.title}</h3><p className="mt-1 text-sm text-[var(--brand-primary)]">Сдать до {deadline(assignment.dueAt, identity.timezone)} · максимум {assignment.maxScore} б.</p></div></div>{assignment.description && <p className="mt-4 text-[var(--text-muted)]">{assignment.description}</p>}{assignment.questions.length > 0 && <ol className="mt-5 space-y-3">{assignment.questions.map((question, index) => <li className="rounded-xl bg-black/5 p-4" key={question.id}><small className="font-bold text-[var(--text-muted)]">Задание {index + 1}{question.topic ? ` · ${question.topic}` : ""} · сложность {question.difficulty}/3</small><p className="mt-2 font-medium">{question.prompt}</p></li>)}</ol>}{assignment.submission ? <HomeworkSubmissionForm assignmentId={assignment.id} lessonId={lesson.id} answer={assignment.submission.answer} status={assignment.submission.status} /> : <StartHomeworkForm assignmentId={assignment.id} lessonId={lesson.id} />}{assignment.submission?.score !== null && assignment.submission?.score !== undefined && <p className="mt-3 text-sm font-bold">Оценка: {assignment.submission.score} из {assignment.maxScore}</p>}</article>)}</div> : <div className="student-empty"><ClipboardCheck aria-hidden="true" /><b>ДЗ пока нет</b><p>Если преподаватель добавит задание к уроку, оно появится здесь вместе с дедлайном.</p></div>}</section>
        </div>
