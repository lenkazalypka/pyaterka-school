...
const documents = {
  ...
  consent: { title: "Согласие на обработку персональных данных", paragraphs: ["Это рабочий черновик согласия, а не финальная юридическая редакция.", "При сохранении публичного маршрута пользователь передаёт имя, один контакт, класс, цель и выбранные предметы для обратной связи. При регистрации дополнительно передаются данные, необходимые для аккаунта и онбординга.", "До включения сбора заявок должны быть утверждены оператор, сроки хранения, порядок отзыва согласия и отдельная процедура для несовершеннолетних, если она требуется."] },
